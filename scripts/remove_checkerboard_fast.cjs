  ...
  .on('parsed', function () {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        // Is Cat Feature Pixel?
        const isBlueScarf = (b > 120 && b > r + 20 && b > g + 15);
        const isCyanEye = (b > 140 && g > 90 && r < 120);
        const isWarmFur = (r > 150 && g > 120 && (r - b) > 10);
        const isDarkEarOrTail = (r > 30 && r < 140 && g > 15 && g < 100 && b > 10 && b < 90 && (r - b) > 10);

        const isCat = isBlueScarf || isCyanEye || isWarmFur || isDarkEarOrTail;
    ...
    cropped.pack().pipe(outStream).on('finish', () => {
      ...
      console.log(`Fast background removal complete! Isolated Cat: ${cropWidth}x${cropHeight}`);
    });
  });
